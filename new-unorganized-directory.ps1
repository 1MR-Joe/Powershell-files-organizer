function New-Unorganized-Directory {
    ...
    $mainDirectory = $Path + "\unorganized-dir"
    ...
    for ($i = 0; $i -lt $DirectoryCount; $i++) {
        ...
        # or create directory
        New-Item -ItemType Directory -Path $subDirectoryName -Force | Out-Null
    }

    
    if($DirectoryCount -gt 0) {
        # loop over sub directories and create files in them
        $maxFilesInDirectory = [System.Math]::Floor($FileCount / 3)
...
$fileCount = Read-Host "Enter file count"
$dirCount = Read-Host "Enter directory count"
$path = Read-Host "Enter path"
New-Unorganized-Directory -FileCount $fileCount -DirectoryCount $dirCount -Path $path